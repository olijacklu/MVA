...
def compute_kernel_pca(Y, K, kernel="rbf", gamma=None):
    kpca = KernelPCA(n_components=K-1, kernel=kernel, gamma=gamma, random_state=42)
    transformed_data = kpca.fit_transform(Y.T)

    top_eigenvalues = np.var(transformed_data, axis=0)

    return top_eigenvalues, transformed_data


'''
# compute_kernel_pca replaces the two original functions to get the K-1 eigenvectors
...
def run_clustering(X, K, kernel="rbf", gamma=None):
    Y = center_data(X)

    top_eigenvalues, top_eigenvectors = compute_kernel_pca(Y, K, kernel=kernel, gamma=gamma)
